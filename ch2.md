...
从这个例子不难看出，计算多重线性变换的叠加，同计算单个线性变换对于一组坐标向量的影响是相同的。毕竟，坐标系中的扭曲的，或是未扭曲的标准坐标向量们也是坐标向量。我们当然也可以通过一次或是多次坐标变换来将扭曲的空间还原。例如：$\begin{bmatrix} \overgroup{0}&\overgroup{-2}\\\undergroup{2}&\undergroup{0}\end{bmatrix}$是将空间左转90度，并将空间放大一倍。那么我们就可以：
